...
while((e1 > e0) or (e2 > e0)):
    for i in range(1,n1-1):
        for j in range(1,n2-1):
            ...
            x2[i][j] = 0.5 * [a * (X[i+1][j] + X[i-1][j]) + g * (X[i][j+1] + X[i][j-1]) - 0.5 * b * (X[i+1][j+1]+X[i-1][j-1]-X[i+1][j-1]-X[i-1][j+1])] / (a + g)
            y2[i][j] = 0.5 * [a * (Y[i+1][j] +  Y[i-1][j]) + g * ( Y[i][j+1] +  Y[i][j-1]) - 0.5 * b * (Y[i+1][j+1]+ Y[i-1][j-1]- Y[i+1][j-1]- Y[i-1][j+1])] / (a + g)
            e1 = abs(X[i][j] - x2[i][j])
            e2 = abs(Y[i][j] - y2[i][j])
